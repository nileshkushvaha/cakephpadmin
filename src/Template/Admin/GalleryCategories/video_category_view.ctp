 ...
 */
$this->assign('title',__('Video Gallery Category ( ' . $galleryCategory->title . ' ) '));
$this->Breadcrumbs->add(__('Video Gallery Categories'),['action'=>'video-category']);
$this->Breadcrumbs->add(__('Video Gallery Category ( ' . $galleryCategory->title . ' ) '));
?>
        ...
        <div class="col-md-12">
           <div class="box box-solid">
              <div class="box-header with-border">
                 ...
                 <h3 class="box-title"><?= __('Video Gallery Category ( ' . $galleryCategory->title . ' ) ') ?></h3>
                 <div class="box-tools pull-right">
                  <?=$this->Html->link(
                    '<i class="glyphicon glyphicon-arrow-left"></i>',
                    ['action' => 'video-category'],
                    ['class' => 'btn btn-info btn-xs','title' => __('Back to Video Gallery Category'),'escape' => false]
                  );?>
                  <?= $this->Html->link('<i class="glyphicon glyphicon-pencil"></i>', ['action' => 'video-category-edit', ],['class' => 'btn btn-warning  btn-xs', 'title' => __('Edit'), 'escape' => false]) ?>

                  <?= $this->Form->postLink(
                    '<i class="fa fa-trash-o"></i>',
                    ['action' => 'video-category-delete', $galleryCategory->id],
                    ['confirm' => __('Are you sure you want to delete this Video Gallery Category?', $galleryCategory->id),'class' => 'btn btn-danger btn-xs','title' => __('Delete'),'escape' => false]
                )?>
              </div>
              </div>
              <!-- /.box-header -->
